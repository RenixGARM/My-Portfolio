                            ...
                            return (
                                <div key={work.id} className="work-card group relative block w-full py-20 cursor-pointer overflow-hidden" onClick={() => setSelectedProject(work)}>
                                    ...
                                    <div className="relative z-10 w-full h-full flex justify-center items-center">
                                        {work.device === "mobile" ? (
                                            <MobileSlider images={work.images} />
                                        ) : (
                                            <LaptopSlider images={work.images} />
                                        )}
                                    </div>

                                    {/* Hover Titles */}
                                    <div className="absolute inset-0 p-8 md:p-12 flex flex-col justify-center items-center pointer-events-none z-20">
                                        <div className="translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 ease-out bg-background/80 backdrop-blur-md p-6 md:p-8 rounded-2xl border border-foreground/5 mt-auto">
                                            <h3 className="text-3xl font-display font-light text-center">{work.title}</h3>
                                            <p className="text-foreground font-sans font-normal text-center mt-2">{work.description}</p>
                                        </div>
                                    </div>
                                </div>
                            );
